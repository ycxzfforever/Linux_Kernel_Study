...
unsigned int memdev_poll(struct file *filp, poll_table *wait)
{
    ...
    if (count)             mask |= POLLOUT | POLLWRNORM; /* writable */
                ...
                loff_t *f_pos)
{
    ...
    *f_pos += count;
    ...
    dev->remain_size += count;
    
    wake_up_interruptible(&dev->outq);    /* 唤醒等待写的进程 */
...
int memdev_init_module(void)
{
    ...
    for (i=0; i < MEMDEV_NR_DEVS; i++) 
    {
        ...
        mem_devices[i].remain_size = MEMDEV_SIZE;
